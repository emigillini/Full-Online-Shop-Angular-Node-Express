...
export class ProductService {
  ...
  async get_random_product_excluding_id(
    ...
    try {
      const product = await prodman.get_random_product_excluding_id(
        current_product_id
      );
      return product;
    } catch (error) {
      console.error(
        "Error in ProductService getRandomProductExcludingId:",
        error
      );
  ...
  async getProductsByBrandName(brandName: string) {
    try {
      ...
      throw new Error(
        `Error fetching products by brand name: ${error.message}`
      );
    }
  }
}

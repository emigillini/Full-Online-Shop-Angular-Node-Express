...
export class MessageManager {
    ...
    async getAllMessages(): Promise<IMessage[]> {
        try {
            return await MessageModel.find().populate("conversation").populate("user").exec();
        } catch (error) {
    ...
    async getMessagesByConversation(conversationId: string): Promise<IMessage[]> {
        try {
            return await MessageModel.find({ conversation: conversationId }).populate("conversation").populate("user").exec();
        } catch (error) {
    ...
    async createMessage(conversation: Types.ObjectId, user: Types.ObjectId, content: string): Promise<IMessage> {
        try {
            ...
            const populatedMessage= await MessageModel.findById(newMessage._id).populate("conversation").populate("user").exec()

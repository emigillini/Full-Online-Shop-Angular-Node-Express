...
export class ApiFallbackInterceptor implements HttpInterceptor {
  ...
  private readonly remoteBaseUrl =
    'https://fullonlline-express-server-production.up.railway.app/api';
    ...
    return next.handle(localReq).pipe(
      catchError((error: HttpErrorResponse) => {
        if (error.status === 0 || error.status === 503) {
          console.info(
            `Error connecting to local URL: ${this.localBaseUrl}. Retrying with remote URL...`
          );

          const remoteReq = this.cloneRequestWithBaseUrl(
            req,
            this.remoteBaseUrl
          );
          return next.handle(remoteReq).pipe(
            tap(() =>
              console.info(
                `Successfully connected to remote URL: ${this.remoteBaseUrl}`
              )
            ),
  ...
  private cloneRequestWithBaseUrl(
    req: HttpRequest<any>,
    baseUrl: string
  ): HttpRequest<any> {
